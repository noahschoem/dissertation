...
\begin{corollary}\label{corollary-superproper-iterate-proper}
  Suppose $\rho$ is a cardinal less than $\delta$ and $\seq{\mathbb{P}_\alpha \mid \alpha<\rho}$ is an inverse support iteration of posets such that for every $\alpha<\rho$ and every $M'\prec (H_\theta,\in,\mathbb{P}_\alpha)$, $1_{\mathbb{P}_\alpha}$ is an $(M',\mathbb{P}_\alpha)$-master condition for $\mathbb{P}_\alpha$.
...
\begin{proof}
  Let $M$ be such, and let $D\in M$ be dense in $\mathbb{P}$.
  Then the projection $D \upharpoonright \mathbb{P}_\alpha$ is dense in $\mathbb{P}_\alpha$ for each $\alpha$, so by assumption $(D \upharpoonright \mathbb{P}_\alpha) \cap M$ is nonempty, as witnessed by $p_\alpha$.
