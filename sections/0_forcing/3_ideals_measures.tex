...
\begin{proposition}
  ...
  \begin{itemize}
    ...
    \item $M$ is closed under $\kappa$-sequences from $V$, i.e. $V\models M^\kappa \subseteq M$
  \end{itemize}
\end{proposition}
...
\begin{proposition}
  ...
  Thus in $V[G]$ we may define $\Ult(V,G)$ and obtain an elementary embedding $j_{\mathcal{I}}:V\to \Ult(V,G)$.
...
\begin{fact}
  For every stationary $S$, $\mathcal{B}_{NS_\kappa}$ is not $\kappa$-saturated below $S$.
\end{fact}

This result can be argued purely using generic ultrapowers. The proof here is in large part courtesy of \cite{billchen_solovay_split}:
\begin{proof}[Proof sketch]
  ...
  Then a $V$-generic filter $G$ for $\mathcal{B}_{NS_\kappa \upharpoonright T}$ is a $V$-$\kappa$-complete $V$-normal $V$-ultrafilter with well-founded ultrapower $j:V\to \Ult(V,G)$ that is closed under $\kappa$-sequences from $V[G]$. 
  Since we forced with $\mathcal{B}_{NS_\kappa \upharpoonright T}$, $T\in G$, hence $\kappa\in j(T)$,
  ...
  A similar argument holds if $S \cap \cof(\lambda)$ is stationary for some $\lambda<\kappa$; for if we force in $\mathcal{B}_{NS_\kappa}$ below $S \cap \cof(\lambda)$, we get that $\kappa\in j(S\cap \cof(\lambda))$ and thus $\cf^{\Ult(V,G)}(\kappa)=\lambda$.
  But $\mathcal{B}_{NS_\kappa}$ is $\kappa$-cc, so preserves the regularity of $\kappa$; this is a contradiction.
\end{proof}

However, there are still useful arguments that can be written \emph{just} from having that $\mathcal{B}_{NS_\kappa}$ is precipitous.
...
Then Foreman's Duality Theorems (the many forms of which can be found in \cite{foreman_ideals_gen_ees}, Chapter 7.4) allow for the computation of various (including saturation) properties of $\overline{\mathcal{I}}$.
We will be using special cases of this fairly general formulation, which appears as Theorem 7.14 of \cite{foreman_ideals_gen_ees}:

\begin{theorem}
  Let $\mathcal{I}$ be precipitous on a set algebra $A$, $\mathbb{P}$ is a poset, and there is a condition $\dot{m} \in \dot{j_{\mathcal{I}}(\mathbb{P})}$ such that the embedding
  \[
    id \times \dot{j}_{\mathcal{I}}: (A / \mathcal{I}) \times \mathbb{P} \to (A / \mathcal{I}) * \dot{j_{\mathcal{I}}(\mathbb{P})} / \dot{m}
  \]
  is regular.
  
  Then there exist conditions $R\in \mathbb{P} * \dot{A/\overline{\mathcal{I}}}$ and $S\in (A/\mathcal{I}) * \dot{j_{\mathcal{I}}(\mathbb{P})}$ such that
  \[
    \left(\mathbb{P} * \dot{A/\overline{\mathcal{I}}}\right)/R \cong
    \left(A/\mathcal{I} * \dot{j_{\mathcal{I}}(\mathbb{P})}\right)/S
  \]
\end{theorem}
