...
\begin{theorem}[Łoś' Theorem]
  For any formula with parameters $\phi(\overrightarrow{x})$, for any $\overrightarrow{[f]}$, $Ult(V,U)\models \phi(\overrightarrow{[f]})$ if and only if for $U$-many $\alpha$, $V\models \phi(\overrightarrow{f(\alpha)})$.
\end{theorem}
...
\begin{proposition}
  ...
  Further, if $\mathcal{I}$ is $\kappa$-complete then $G$ is $V$-$\kappa$-complete, and if $\mathcal{I}$ is normal then $G$ is $V$-normal.
\end{proposition}
...
\begin{caveat}
  In contrast to the definition for a general poset in (TODO: x-ref),
...
\begin{proof}[Proof sketch]
  Suppose for sake of contradiction that $NS_\kappa$ is $\kappa$-saturated.
  Then a $V$-generic filter $G$ for $\mathbb{B}_{NS_\kappa}$ is a $V$-$\kappa$-complete $V$-normal $V$-ultrafilter with wellfounded ultrapower $Ult(V,G)$.
...
As for other results about the nonsaturation of nonstationary ideals, Gitik and Shelah, in \cite{gitik_shelah_less_sat} showed that when $\kappa>\omega_1$, $NS_\kappa$ is not $\kappa^+$-saturated.
However, it is consistent (e.g. in the presence of Martin's Maximum, c.f. \cite{foreman_magidor_shelah_martin_max}) that $NS_{\omega_1}$ is $\omega_2$-saturated.
...
Burke, Foreman, Gitik, Magidor, Matsubara, and Shelah proved this over much time and many papers; a summary and the proof of the case $\kappa=\lambda=\omega_1$ can be found in \cite{foreman_magidor_ns_ns}.
We will discuss and extend methods from \cite{foreman_magidor_ns_ns} in Chapters \ref{ms} and \ref{accessible_ms}.
...
Precipitousness results concerning nonstationary ideals have a long history;
the precipitousness of $NS_\kappa$ is consistent relative to a measurable for $\kappa=\omega_1$.
Consistency results can be found in \cite{cummings_itd_forcing_ees}) and  \cite{foreman_ideals_gen_ees}.
