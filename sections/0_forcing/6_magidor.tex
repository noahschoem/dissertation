...
\begin{proof}
  As the collection of conditions deciding $\sigma$ is open dense, by \thref{magidor_prikry_type_open_dense_better}, let $(f,B)\leq^* (f,A)$ be such that $(f,B)_\beta \leq^* (f,A)_\beta$ and whenever $(g,H)\leq (f,B)$ and $(g,H) \compat \sigma$, then also $(g,H)_\beta \frown (f,B)^\beta \compat \sigma$.
  But since $(g,H)\leq (g,H)_\beta \frown (f,B)^\beta$, $(g,H)_\beta \frown (f,B)^\beta$ must decide $\sigma$ the same way as $(g,H)$.
\end{proof}
