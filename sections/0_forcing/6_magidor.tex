...
\begin{lemma}[Prikry-type lemma, head-change version]
...
\begin{lemma}[Prikry-type lemma, head-change sentential version]\thlabel{magidor_prikry_type_sentential_better}
...
\begin{proof}
  ...
  But then
  \[
    \mathcal{D}' := \left\{(p)_{\lambda_\rho} \mid p\in \mathcal{D}, \ (p)^{\lambda_\rho}\leq(\emptyset,B)^{\lambda_\rho}, \rho\in dom(stem(p)), \text{ and }stem(p)(\rho)=\lambda_\rho\right\}
  \]
  is open dense in $\mathbb{M}\left(\vec{U}\right)_{(\rho,\lambda_\rho)}$, and since $\vec{\beta}\upharpoonright \rho$ is geometric for $\mathbb{M}\left(\vec{U}\right)_{(\rho,\lambda_\rho)}$,
  by the induction hypothesis let $(f,A)$ be such that $(f,A)_{\lambda_\rho}\in H_\rho\cap \mathcal{D}'$.
  In particular, this means that $f\upharpoonright \rho\subseteq \vec{\beta}\upharpoonright \rho$,
  for all $\xi\in dom(A)\cap \rho$, $\beta_\xi\in A(\xi)$,
  and for all $\xi\in dom(f)\setminus \lambda_\rho$, $f(\xi)\in B(\xi)$.
  
  \begin{comment}let $a_g$, $A_g$ be such that every $a_g$-step extension of $(g,A_g)$ is in $\mathcal{D}$.
...
(N. B. This is corollary 7.6 of Fuchs.)
