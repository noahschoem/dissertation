...
The original approach for Magidor forcing from (todo: cite magidor, chg unctble cof) uses a system of representatives $r$ for normal measures below $\kappa$ the measurable cardinal we wish to singularize to cofinality $\lambda<\kappa$, and follows from a coherent system of measures:
...
Additionally, we will say that $A$ is the \emph{measure component} of $p$ and write $meas(p)$ to mean $A$\footnote{
  We will use an alternative convention $p=(f^p,A^p)$ in line with (todo: cite gitik HBC) in a future section, when $stem$ and $meas$ become too cumbersome.
}.

\begin{remark}\thlabel{magidor_weakest_step_extension}
...
$\mathbb{M}\left(\vec{U}\right)$ itself factors into different forcings below each point, as follows:

\begin{definition}
  Let $p=(f,A)\in \mathbb{M}\left(\vec{U}\right)$ and let $\xi<\lambda$.
  Then we may define the following:
  \begin{itemize}
    \item $p_\xi=(f,A)_\xi=(f\upharpoonright (\xi+1),A\upharpoonright (\xi+1))$
...
\begin{remark}\thlabel{magidor_factoring}
  Observe that by choice of $Y_\xi$'s, relative to the weakest $A$ for which $p=((\xi,\beta),A)$,
  we have that $\mathbb{M}\left(\vec{U}\right) / p\cong \mathbb{M}\left(\vec{U}\right)_{\xi,\beta} \times \mathbb{M}\left(\vec{U}\right)^{\xi,\beta}$.
  In particular, in $V$, $\mathbb{M}\left(\vec{U}\right)_{\xi,\beta}$ has the $\beta^+$-cc and $(\mathbb{M}\left(\vec{U}\right)^{\xi,\beta},\leq^*)$ is $<\beta^+$-directed closed.
  
  Additionally, by (todo: cite Easton's Lemma that for cc x closed, closed remains distributive after cc forcing. Proof is exactly as in Jech, p. 234.), after forcing with $\mathbb{M}\left(\vec{U}\right)_{\xi,\beta}$, the partial order $(\mathbb{M}\left(\vec{U}\right)^{\xi,\beta},\leq^*)$ remains $<\beta^+$-distributive.
\end{remark}
...
\begin{proof}
  ...
  Since $D$ is open dense, let $q\in D$ be below $r$, let $a$ be such that $q$ is an $a$-step extension of $r$.
...
\begin{corollary}[Prikry-type lemma, sentential version I]\thlabel{magidor_prikry_type_sentential}
...
Further arguments along similar lines yields an even stronger sentential Prikry-type lemma:
\begin{lemma}[Prikry-type lemma, sentential version II]\thlabel{magidor_prikry_type_sentential_better}
...
\end{lemma}
This appears as Lemma 4.5 of (todo: cite Magidor, chg cofs), and we give a proof here:

\begin{proof}
  For ease of notation, let $f(\beta)=\rho$.
  For each $r\leq (f,A)_\beta$ in $\mathbb{M}(\vec{U})_{(\beta,\rho)}$, by \thref{magidor_prikry_type_sentential}, let $p_r\leq^* r \frown (f,A)^\beta$ (in the full $\mathbb{M}(\vec{U})$) be such that $p_r$ decides $\sigma$.
  ...
  \[
    ...
    \end{cases}
  \]
  To complete the argument, if $(g,H)\leq (f,B)$ and $(g,H)$ decides $\sigma$, let $r=(g,H)_\beta$.
  Then by construction, $(g,H)\leq r \frown (f,B)^\beta\leq p_r$ which decides $\sigma$, and thus $(g,H)$ and $(g,H)_\beta \frown (f,B)^\beta$ must decide $\sigma$ the same way.
\end{proof}
...
The proof is also in (Magidor, chg cofs, TODO cite).
...
\begin{proposition}\thlabel{magidor_generic_implies_geometric}
...
(TODO: flesh out the limit reflection argument. Say why it's the same. Can do the special case of $g=((\xi,\beta_\xi))$, and spell it out by naming the relevant dense set.)
\begin{proof}
  When $\xi=\lambda$, $\{(g,H) \mid \forall \eta>\max(dom(g)), \ H(\eta)\subseteq A_\eta\}$ is open dense by measure-one set intersection.
  ...
  \]
  is open dense in $\mathbb{M}(\vec{U})_{(\xi,\beta_\xi)}$, hence is open dense below the weakest condition $p$ in the full $\mathbb{M}(\vec{U})$ with $stem(p)(\xi)=\beta_\xi$.
\end{proof}

The converse is also true in the following sense:
\begin{proposition}\thlabel{magidor_geometric_iff_generic}
  ...
  \[
    ...
    \left\{(f,A) \middle|
      \begin{array}{l}
        f\text{ is a finite subsequence of }\vec{\beta}, \\ dom(A)=\lambda\setminus dom(f), \\        
        \forall \eta \in dom(A) \ A(\eta)\in U_\eta \text{ or the relevant }r^\gamma_\eta(\beta_\gamma), \\
        \text{ and }\beta_\eta\in A(\eta)
      \end{array}
    \right\}
  \]
...
A proof can be found in (Fuchs, todo cite) that makes use of much stronger Prikry-type lemmas than \thref{magidor_prikry_type_open_dense} and \thref{magidor_prikry_type_sentential_better};
these stronger Prikry-type lemmas handle the cases where for a particular $(f,A)$ and $\seq{B_\eta \mid \eta<\xi}\in V$, there are nonzero (but coboundedly many) $\eta\in dom(A)$ but $\beta_\eta\notin B_\eta$.
We present an alternative proof here:

(TODO: prove that this is a filter.
TODO: also try proving genericity. Perhaps by induction on $\lambda$; try the case of $\lambda=\omega$, see if you can induct to $\omega+\omega$, and then limit of limits such as $\omega^2$.)

\begin{proof}
  $H$ is a filter, as by construction $(\emptyset,\seq{Y_\xi\mid \xi<\lambda}\in H$ and $H$ is clearly upwards closed. 
  ...
  Let $\vec{Y}=\seq{Y_n\mid n<\omega}$ be as in \thref{mitchell_rep_coherence_sets}.
  ...
  by \thref{magidor_prikry_type_open_dense}, let $a_g$, $A_g$ be such that $(g,A_g)\leq^* (g,\vec{Y}\upharpoonright (\omega\setminus dom(g)))$ and every $a_g$-step extension of 
  ...
  For the successor step, suppose $\lambda=\bar{\lambda}+\omega$ for some limit $\bar{\lambda}$ such that the result is true for any $\mathbb{M}\left(\seq{U_\eta\mid \eta<\bar{\lambda}}\right)$.
  ...
  By geometricity and induction, the resulting $H_g$ defined from $\seq{\beta_\eta \mid \eta<\bar{\lambda}}$ is $\mathbb{M}\left(\seq{U_\eta\mid \eta<\bar{\lambda}}\right)$-generic over $V$; since $\mathbb{M}\left(\seq{U_\eta\mid \eta<\bar{\lambda}}\right)$ is $\beta_{\bar{\lambda}}^+$-cc, and by our base case, the filter $H^g$ defined from $\seq{\beta_{\bar{\lambda}+n}\mid n<\omega}$ is $\mathbb{M}\left(\seq{U_{\bar{\lambda}+n} \mid n<\omega}\right)$-generic over $V[H_g]$.
  So by the Product Lemma, $H=H_g\times H^g$ is $\mathbb{M}\left(\seq{U_\eta \mid \eta<\lambda}\right)$-generic over $V$.
  
  For the limit case, suppose that $\lambda=\sup_{\rho<\tau} \lambda_\rho$ for some $\tau\leq\lambda$ regular.
  Further suppose that for all $\rho<\tau$, $\vec{\beta}\upharpoonright \lambda_\rho$ induces $H_\rho$ an $\mathbb{M}\left(\vec{U}\right)_{(\rho,\beta_{\lambda_\rho})}$-generic over $V$ (and by size bounds, over intermediate extensions as well) (with product decomposition as above).
  Let $\mathcal{D}$ be open dense; for each $\mathbb{M}\left(\vec{U}\right)$-stem $g$, by \thref{magidor_prikry_type_open_dense}, let $a_g$, $A_g$ be such that every $a_g$-step extension of $(g,A_g)$ is in $\mathcal{D}$.
%   Let $B$ be a measure one system for $\mathbb{M}\left(\vec{U}\right)$ given by
%   \[
%     B(\eta)=\diagcap_g A_g(\eta)
%   \]
  ...
  By assumption on $\vec{\beta}$, $\vec{\beta}\upharpoonright \lambda_\rho$ is geometric for $\mathbb{M}\left(\vec{U}\right)_{(\lambda_\rho,\beta_{\lambda_\rho})}$ and thus $H_{\rho}$ is $\mathbb{M}\left(\vec{U}\right)_{(\lambda_\rho,\beta_{\lambda_\rho})}$-generic over $V$; however, (something's still not right. how decompose and argue meets $\mathcal{D}$?)
\end{proof}


At the very least, our tools yield the following lemma that says geometric sequences actually meet $V$-measure one systems cofinitely instead of just coboundedly.
This helps for arguing mutual stationarity results for restricted cofinality below $\beta_0$, by a lemma from the original mutual stationarity paper of (foremanmagidor-nsns, todo cite).
