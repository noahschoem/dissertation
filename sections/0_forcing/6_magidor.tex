...
\begin{itemize}
  ...
  \item A condition $(f,A)$ in the generic filter has specified a finite subset of the above sequence where if $\eta\in dom(f)$, then $f(\eta)=\alpha_\eta$. Further, if $\eta\in dom(A)$, then $(f,A)$ specifies that $\alpha_\eta\in A(\eta)$ (and in particular, $\alpha_\eta$ is a $V$-measurable cardinal with $\vec{U}$-rank $\eta$).
\end{itemize}
...
\begin{definition}\thlabel{magidor_condition_projection}
  Let $p=(f,A)\in \mathbb{M}\left(\vec{U}\right)$ and let $\xi$ be a limit ordinal below $\lambda$.
...
\begin{fact}\thlabel{magidor_factoring}
  Observe that, for $\xi$ limit, by choice of $Y_\xi$'s, relative to the weakest $A$ for which $p=((\xi,\beta),A)$,
...
Before we state and prove Prikry-type lemmas for Magidor forcing, it is worth formalizing how we may diagonalize over all possible extensions.
...
\begin{definition}\thlabel{magidor_weakest_step_extension}
  If $p=(f,A)$, $\vec{\nu}:a\to \kappa$ for some $a$ a finite subset of $\lambda\setminus dom(f)$, and there is some $B$ for which $(f\frown \vec{\nu},B)$ is a condition below $p$,
  ...
  \begin{itemize}
    \item if $\tau\in dom(\nu)$, we have that $A'(\xi)=A(\xi)\cap (\vec{\nu}(\tau))$; since $[\alpha \mapsto A(\xi) \cap \alpha]_{r_\xi^\tau (\nu(\tau))}=A(\xi)$ and $\nu(\tau)\in Y_\tau$, $A'(\xi)\in r_\xi^\tau (\nu(\tau))$ and is the $\subseteq$-largest such.
...
\begin{proof}
  ...
  \begin{claim}\thlabel{magidor_diagonalize_empty_one_step}
  ...
  \begin{proof}[Proof of claim]
    ...
    \begin{enumerate}[label={Case \arabic*:},labelindent=15pt]
      ...
      By Łoś' Theorem and definition of $r_\beta^\alpha$, $[\nu\mapsto H_\nu(\beta)]_{U_\alpha}\in U_\beta$; let $B_{\beta,\alpha}=[\nu\mapsto H_\nu(\beta)]_{U_\alpha}$.
  ...
  Of course, we will want to argue for arbitrary $g$:
  \begin{claim}\thlabel{magidor_diagonalize_any_one_step}
    For every $g$ and every $\alpha<\lambda$ there is some $p_\alpha\leq^*p$ (with $p_\alpha=(\emptyset,H^\alpha)$) that is good for $p$ and $E_{\{\alpha\}}$, that is, for all $\nu\in H^\alpha(\alpha)$, $p_\alpha \frown (\alpha,\nu)\leq^* p_{(\alpha,\nu)}$.
  \end{claim}
  
  \begin{proof}[Proof of claim]
    As in \thref{magidor_diagonalize_empty_one_step}, for each $\nu\in H(\alpha)$, let $p_{(\alpha,\nu)}\leq^* p \frown (\alpha,\nu)$, where $p_{(\alpha,\nu)}=(g,H_\nu)$.
  
    In the event that $g=(\xi,\rho)$, we have two cases, depending on $\alpha$:
    
    \begin{enumerate}[label={Case \arabic*:},labelindent=15pt]
    
      \item If $\alpha>\xi$, then we may argue as in \thref{magidor_diagonalize_empty_one_step} for $p\upharpoonright (\xi,\lambda)\in\mathbb{M}\left(\seq{U_\gamma \mid \xi<\gamma<\lambda}\right)$ to obtain some 
      \[
        (\emptyset,\tensor*[^\xi]{H}{^\alpha})\in \mathbb{M}(\seq{U_\gamma \mid \xi<\gamma<\lambda})
      \]
      such that whenever $\nu\in \tensor*[^\xi]{H}{^\alpha}(\alpha)$, $(\emptyset,\tensor*[^\xi]{H}{^\alpha}) \frown (\alpha,\nu) \leq^* p_{(\alpha,\nu)} \upharpoonright (\xi,\lambda)$.
      
      As for working below $\xi$, let $\zeta<\xi$.
      Then for each $\nu\in \tensor*[^\xi]{H}{^\alpha}(\alpha)$, $H_\nu(\zeta)\in r_{\zeta}^\xi(\rho)$, and $\lvert r_{\zeta}^\xi(\rho) \rvert=2^{\rho}<\kappa$.
      So by $\kappa$-completeness there is an $F_{\zeta,\alpha}\in U_\alpha$ and some $G_{\zeta,\alpha} \in r_{\zeta}^\xi(\rho)$ such that for all $\nu\in F_{\zeta,\alpha}$, $H_\nu(\zeta)=G_{\zeta,\alpha}$.
      
      Then our desired $p_\alpha\leq^* p$ is given by
      \[
        meas(p_\alpha)(\gamma)=\begin{cases}
          G_{\gamma,\alpha} & \gamma<\xi \\
          \tensor*[^\xi]{H}{^\alpha}(\gamma) \cap \bigcap_{\zeta<\xi} F_{\zeta,\alpha} & \gamma=\alpha \\
          \tensor*[^\xi]{H}{^\alpha}(\gamma) & \gamma>\xi, \ \gamma\neq\alpha \\      
        \end{cases}
      \]
      since whenever $\nu\in meas(p_\alpha)(\nu)$, by construction we have that $(p_\alpha \frown (\alpha,\nu)) \upharpoonright (\xi,\lambda)\leq^* p_{(\alpha,\nu)} \upharpoonright (\xi,\lambda)$, and since $\nu\in F_{\zeta,\alpha}$ for all $\zeta<\xi$, $(p_\alpha \frown (\alpha,\nu)) \upharpoonright \xi=\seq{G_{\gamma,\alpha}\mid \gamma<\xi}$ which is by construction a direct extension of $p_{(\alpha,\nu)} \upharpoonright \xi$.
      
      \item If $\alpha<\xi$, then $H(\alpha)\in r_\alpha^\xi(\rho)$ and hence $H(\alpha)\subseteq \rho$.
      So working in $\mathbb{M}\left(\vec{U}\right)_{(\xi,\rho)}$, we may invoke \thref{magidor_diagonalize_empty_one_step} for $(\emptyset, H) \upharpoonright \xi$ to obtain $(\emptyset,G)\in \mathbb{M}\left(\vec{U}\right)_{(\xi,\rho)}$ such that for every $\nu\in G(\alpha)$, $(\emptyset,G)\frown (\alpha,\nu)\leq^* p_{(\alpha,\nu)}\upharpoonright \xi$.
      But then since there are only $\rho$-many such $\nu$, for each such $\nu$ and each $\beta>\xi$, $\bigcap_{\nu\in G(\alpha)} H_\nu(\beta)\in U_\beta$.
      So let
      \[
        p_\alpha=\left( (\xi,\rho) , G\frown \seq{\bigcap_{\nu\in G(\alpha)} H_\nu(\beta) \mid \xi<\beta<\lambda} \right)
      \]
      Then $p_\alpha\leq^* p$ and by our construction for $p_\alpha \upharpoonright (\xi,\lambda)$, we have that $p_\alpha\frown (\alpha,\nu)\leq^* p_{(\alpha,\nu)}$ for every $\nu\in G(\alpha)$.
      
    \end{enumerate}
    
    For larger $g$, note that $|g|$ is always finite.
    So, letting $\beta=\max dom(g)$, we may recursively argue exactly as above for $p \upharpoonright (\beta,\lambda)$ and $p \upharpoonright \beta$.
  \end{proof}
...
\begin{proof}
  ...
  \begin{claim}\thlabel{magidor_stepwise_density_hom}
    For every $a\in [dom(H)]^{<\omega}$, there is a $q\leq^* p'$ such that either every $a$-step extension of $q$ is in $D$, or none are.
  \end{claim}
  \begin{proof}[Proof of claim]
    ...
    If instead $B'=B^-$, then let $\nu\in B^-$.
    Then $q\frown (i,\nu)\leq^* p'\frown(i,\nu)\leq^* p_{(i,\nu)}$ and since $p'\frown(i,\nu)\notin D$, we must have had that $p_{(i,\nu)}=p\frown (i,\nu)$ which is also not in $D$.
    What's more, by definition of $p_{(i,\nu)}$, \emph{no} direct extension of $p\frown (i,\nu)$ lands in $D$, either.
    In particular, no direct extension of $q\frown (i,\nu)$ is in $D$.
    Since $\nu$ ranged over all valid $i$-step extensions of $q$, we have shown that no $\{i\}$-step extension of $q$ is in $D$.
  ...
  \end{proof}
  By iterated use of the above claim, we fix a $\lambda$-length enumeration of $[dom(H)]^{<\omega}$ and obtain a $\leq^*$-decreasing sequence $\seq{p_a\mid a\subseteq [dom(H)]^{<\omega}}$ with $p_a\leq^* p'$ such that every $a$-step extension of $p_a$ is in $D$ or none are.
  
  Let $r$ be a $\leq^*$-lower bound of the $p_a$'s, obtained by taking $\lambda$-sized intersections over the measure components of the $p_a$'s.
...
A careful reading of the above argument actually gives something stronger:

\begin{lemma}\thlabel{magidor_prikry_type_open_dense_addendum}
  Let $D$ be an open dense set, $p=(g,H)$ be a condition, and $r\leq^* p$ be as in the proof of \thref{magidor_prikry_type_open_dense}.
  Then for every $b\subseteq [dom(H)]^{<\omega}$, either every $b$-step extension of $r$ is in $\mathcal{D}$, or none are.
\end{lemma}
...
The proof may be done by induction: for such $(f,A)$, let $(f,B)\leq^* (f,A)$ and $a\subseteq[\lambda]^{<\omega}$ be such that every $a$-step extension of $(f,B)$ decides $\sigma$.
The argument then proceeds by induction on $|a|$.
But the crux of the argument is really a measure-one concentration:
...
\begin{proof}
  ...
  For each $r\leq (f,A)_\beta$ in $\mathbb{M}(\vec{U})_{(\beta,\rho)}$, by \thref{magidor_prikry_type_open_dense_addendum}, let $p_r\leq^* r \frown (f,A)^\beta$ (in the full $\mathbb{M}(\vec{U})$) be such that for every $b$, either every $b$-step extension of $p_r$ is in $D$, or none are.
  ...
  Since there are at most $2^\rho$-many such $r$ and since $\leq^* $ on $\mathbb{M}(\vec{U})^{(\beta,\rho)}$ is $\rho'$-directed closed for $\rho'$ the least inaccessible above $\rho$, 
  let $(f,B)$ be the greatest $\leq^*$-lower bound of the $q_r$'s, where $B$ is given by
  \[
    B(\gamma)=\begin{cases}
      A(\gamma) & \gamma<\beta \\
...
\end{lemma}
This appears as Lemma 4.5 of \cite{magidor_chg_cof}, and has a short proof:

\begin{proof}
  As the collection of conditions deciding $\sigma$ is open dense, we may re-run the argument of \thref{magidor_prikry_type_open_dense_better}, but while invoking \thref{magidor_prikry_type_sentential} to ensure that each $p_r$ as above also decides $\sigma$.
  Then define $(f,B)$ as in the proof of \thref{magidor_prikry_type_open_dense_better}.
  Then if $(g,H)\leq (f,B)$ and $(g,H)$ decides $\sigma$, then let $r=(g,H)_\beta$.
  As before $(g,H)\leq r \frown (f,B)^\beta\leq p_r$ and $p_r$ already decides $\sigma$. 
  But then $p_r$ must decide $\sigma$ the same way as $(g,H)$, and hence $r \frown (f,B)^\beta=(g,H)_\beta \frown (f,B)^\beta$ must also decide $\sigma$ the same way as $(g,H)$.
\end{proof}
...
\begin{definition}\thlabel{def_magidor_geometricity}
  Suppose in some outer model $W$ of $V$, some sequence $\vec{\beta}=\seq{\beta_\eta \mid \eta<\lambda}\in W$ is increasing, normal, and cofinal in $\kappa$.
...
\begin{lemma}\thlabel{magidor_generic_implies_geometric}
  Let $G$ be $\mathbb{M}\left(\vec{U}\right)$-generic over $V$ and let
  \[
...
\begin{theorem}\thlabel{magidor_geometric_iff_generic}
  Suppose $\vec{\beta}=\seq{\beta_\eta \mid \eta<\lambda}$ is geometric.
  Let 
  \[
    H=
...
We present a more direct proof here, which better generalizes to more exotic Magidor-type forcings such as the one we describe in Chapter \ref{accessible_ms}:
...
\begin{proof}
  ...
  For each $g\in V$ a valid $\mathbb{M}(\seq{U_n\mid n<\omega})$-stem such that $dom(g)$ is an initial subsequence of $\omega$\footnote{
    Since $\lambda=\omega$, we may without loss of generality extend each $g$ whose domain is a finite subsequence of $\omega$ to some $g'$ whose domain is $[0,\max dom(g)]$.
  },
  ...
  For the limit case, suppose that $\lambda=\sup_{\rho<\tau} \lambda_\rho$ for some $\tau\leq\lambda$ regular, where each $\lambda_\rho$ is a limit ordinal;
  our induction hypothesis is now that for all $\rho<\tau$, whenever $\vec{\beta}$ is of length $\lambda_\rho$ and is geometric for some Magidor forcing $\mathbb{M}$ of length $\lambda_\rho$, the resulting filter $H_\rho$ defined from $\vec{\beta}$ is $\mathbb{M}$-generic over $V$.
  Note furthermore that if $\rho<\rho'$ then by the Product Lemma and chain condition, $H_{\rho'}=H_\rho \times H'$ for some $H_\rho$ $\mathbb{M}_{(\lambda_\rho,\beta_{\lambda_\rho})}$-generic over $V$ and some $H'$ $\mathbb{M}^{(\lambda_\rho,\beta_{\lambda_\rho})}$-generic over $V[H_\rho]$.
  ...
  \begin{itemize}
    ...
    \item letting $\delta=\max dom(g)$, $(g,A_g)_\delta = (1_{\mathbb{M}\left(\vec{U}\right)}\frown g)_\delta$\footnote{
      Recall from \thref{magidor_condition_projection} that $(f,A)_\delta = (f \upharpoonright \delta+1, A \upharpoonright \delta+1)$.
    }
    \item (tail-change property) for every $b\subseteq [dom(A_g)]^{<\omega}$, if $(g',H)$ is a $b$-step extension of $(g,A_g)$ and $(g',H)\in \mathcal{D}$ then every $b\setminus (dom(g')\cap \delta)$-step extension of $(g',H)_\delta \frown (g,A_g)^\delta$ is also in $\mathcal{D}$
  \end{itemize}
  ...
  But then
  \[
    \mathcal{D}' := \left\{(f,A)_{\lambda_\rho} \mid (f,A)\in \mathcal{D},\ (f,A)^{\lambda_\rho}\leq (\emptyset,B)^{\lambda_\rho}, \text{ and }f(\lambda_\rho)=\beta_{\lambda_\rho}\right\}
  \]
  ...
  by genericity of $H_\rho$ let $(f,A)\in \mathbb{M}\left(\vec{U}\right)$ be such that $(f,A)\in \mathcal{D}$,
  ...
  Let $(h,C)=((f,A)_{\lambda_\rho} \frown (\emptyset,A_g)^{\lambda_\rho})\frown (\vec{\beta}\upharpoonright b)$.
  Then $(h,C)$ is a $b$-step extension of $(f,A)_{\lambda_\rho} \frown (\emptyset,A_g)^{\lambda_\rho}$, so lies in $\mathcal{D}$;
  by construction of $f$, $h$ is a finite subsegment of $\vec{\beta}$;
  and by construction of $A$, $A_g$, and $B$, for all relevant $\xi$, $\beta_\xi\in C(\xi)$.
  Therefore, $(h,C)\in H$, and also $(h,C)\in\mathcal{D}$ as desired.
\end{proof}
