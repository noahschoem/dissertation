...
This is stated without proof in \cite{foreman_magidor_ns_ns}. 
The proof is fairly straightforward, and proceeds via the use of \emph{indiscernibles}:
...
\begin{theorem}[Theorem 5.4 of \cite{cummings_foreman_magidor_canonical_structure}]\thlabel{ms_prikry}
...
This theorem originally appeared in \cite{cummings_foreman_magidor_canonical_structure}.
But, in ways we will see in \ref{ms:koepke}, \thref{ms_prikry} was anticipated much earlier by results in \cite{koepke_free_subset}.
...
\begin{lemma}[Folklore]
  ...
  \end{enumerate}
\end{lemma}
\cite{foreman_magidor_ns_ns} cites this as implicit in \cite{baumgartner_size_clubs}.
...
As of writing, the best known argument in the other direction comes courtesy of \cite{benneria_singular_stat}:
...
\begin{theorem}[Theorem 1.6 of \cite{koepke_ms_omega_1}; Theorem 1 of \cite{koepke_welch_strength_ms}]
  The principle 
  \[
...
The arguments of \cite{koepke_welch_global_square_ms} use global $\square$ in the Dodd-Jensen core model $K$ to construct inner models for large cardinals from $MS$ principles, but relationships between $MS$ principles and failures of $\square$ remain uninvestigated.
...
\begin{proof}[Proof of \thref{ms_antisat}]
  ...
  We claim that the family $\seq{S_f \mid f\in \prod_{\alpha<\mu} \kappa_\alpha}$ is an antichain for the nonstationary ideal on $\mathcal{P}_\delta(\lambda)$.
  To see this, since $\seq{T^\alpha_{f(\alpha)}\mid \alpha<\mu}$ is mutually stationary, each $S_f$ is stationary in $\mathcal{P}_\delta(\lambda)$.
  To argue that $S_f\cap S_g$ is nonstationary for distinct $f$, $g$, fix $f\neq g$.
  Then there is some $\alpha$ for which $f(\alpha)\neq g(\alpha)$; hence for such $\alpha$, $S^\alpha_{f(\alpha)}\cap S^\alpha_{g(\alpha)}=\emptyset$.
  Thus $S_f\cap S_g$ misses the club $\{M\in \mathcal{P}_\delta(\lambda)\mid \alpha\in M\}$.
  
  $\seq{S_f \mid f\in \prod_{\alpha<\mu} \kappa_\alpha}$ is an antichain for the nonstationary ideal on $\mathcal{P}_\delta(\lambda)$, of cardinality $\prod_{\alpha<\mu} \kappa_\alpha=\lambda^\mu$.
\end{proof}

However, very little is known about mutual stationarity principles with $\lnot SCH$.

The argument of \cite{koepke_ms_omega_1} does not depend on cardinal arithmetic, so should readily achieve $MS(\seq{\aleph_{2n+3}\mid n<\omega; cof(\leq\omega_1)}) + \lnot SCH_{\aleph_\omega})$.

The arguments of \cite{benneria_singular_stat} assume $GCH$, but suggest a framework for achieving mutual stationarity principles with the failure of $GCH$.
