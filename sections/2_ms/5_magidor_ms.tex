...
\begin{proof}
  ...
  Since $\vec{U}$ is a family of normal measures on $\kappa$,  an application of \thref{simult_indisc_useful} yields a family $X\in \prod_{\max dom(g)<\alpha<\lambda} U_\alpha$ of order indiscernibles for $F$.
  ...
  Suppose $x,y\subseteq \left[\bigcup_{\alpha<\lambda}A_\alpha\right]^{<\omega}$ are such that $type(x)=type(y)$ and $x\cap (\dot{f}[G](x)+1)=y\cap(\dot{f}[G](x)+1)$.
  ...
  Since $x$, $y$, and $g'\upharpoonright (\delta,\lambda)$ were all picked from $X$, by choice of $X$ we have that $F(g',x)=F(g',y)$ and therefore $f(x)=f(y)$.
\end{proof}
