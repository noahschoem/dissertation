...
\begin{lemma}\thlabel{mag_forcing_preserves_stationary_successors}
  Let $\delta$ be a cardinal in $V$, let and suppose that in $V[\mathbb{M}\left(\vec{U}\right)]$, $\delta=\kappa_{\alpha+1}$ for some $\alpha$.
...
\begin{proof}
  ...
  Since the direct order on $\mathbb{M}\left(\vec{U}\right)^{(\alpha+1,\delta)}$ remains $\delta$-closed after forcing with $\mathbb{M}\left(\vec{U}\right)_{(\alpha+1,\delta)}$, $S$ remains stationary in the full generic extension.
\end{proof}
...
\begin{proof}
  ...
  Let $\mathcal{M}$ be a transitive structure possessing Skolem functions for itself, containing $\kappa$, $\vec{U}$, $\mathbb{M}\left(\vec{U}\right)$, $(g,H)$, and $\dot{f}$ as constants, $\kappa\subseteq \mathcal{M}$, and reflecting enough of $V$ for the following argument.\footnote{
  ...
  \]
  Let $G'$ be $\mathbb{M}\left(\vec{U}\right)$-generic over $V$ with $(g,Y)\in G'$; let $\seq{\kappa'_\alpha \mid \alpha<\lambda}$ be the $G'$-generic Magidor sequence.
