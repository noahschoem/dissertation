...
Koepke's mutual stationarity result in (todo: cite koepke_omega1) implicitly rephrases \thref{ms_prikry} as a Ramsey-theoretic result, in the following sense:

\begin{definition}[Definition 2.2 of (todo: cite koepke_omega1)
  ...
  \begin{enumerate}
    \item Let $x\in [\kappa]^{<\omega}$. Then $type(x)=\seq{|x \cap \kappa_\alpha| \mid \alpha\in \lambda}$. 
...
\begin{remark}
  \begin{enumerate}
    ...
    The idea here is that a sequence is mutually homogeneous for $F$ if $F$'s values on the (finite subsets picked from the) sequence depend only on type and a generalization of regressiveness. In a sense, this is exactly the behavior one would want out of a Skolem function on a structure with many large order-indiscernibles, which is exactly how the (todo: cite Foreman, Magidor, Canonical Structures II) result proceeds.
\end{remark}
...
Our use case for mutually homogeneous sequences is to build a form of indiscernible sequence for a given model $\mathcal{M}$:
...
\begin{remark}\thlabel{rmk_construct_mh_for_structure}
  If $\seq{\kappa_\alpha \mid \alpha\in \lambda \cap Succ}$ is mutually Ramsey such that each $I_\alpha\subseteq \kappa_\alpha$ may be chosen to be club (or measure one in $V$ for some appropriate notion of ``measure one") in $\kappa_\alpha$,
  ...
  So to obtain a mutually homogeneous system for $\mathcal{M}$, we construct mutually homogeneous systems of clubs for $F$ and for each $\phi$ and each $g$ as in \thref{mutually_homogeneous_for_structure}, of which there will be less than $\kappa_0$-many, and take their intersection.
\end{remark}

The (Foreman, Magidor, Canonical Structures II) result on Prikry generics fundamentally relies only on arguments involving such $F:[\kappa]^{<\omega}\to \kappa$. 
Koepke has arguments pertaining to such $F$ and in a much earlier paper proved the following:

\begin{theorem}[Koepke 1984; also Koepke 2007, Proposition 2.3]\thlabel{koepke_mr}
...
The proof is fundamentally exactly as in (Foreman, Magidor, Canonical Structures II) but Koepke reproves it anyways.
