...
And as with vanilla Magidor forcing, we obtain the sentential versions of the Prikry lemma:
...
A similar tail-change lemma applies equally well:

\begin{lemma}[Prikry-type lemma, sentential version II]\thlabel{magidor_collapse_prikry_sentential_better}
  Let $\sigma$ be a sentence of the forcing language, let $p\in \mathbb{MC}(\vec{U})$, and let $\beta\in dom(f^p)$.
  Then there is some $q\leq^* p$ such that $q\upharpoonright \beta=p\upharpoonright \beta$ and if $r\leq q$ such that $r$ decides $\sigma$, then $(r\upharpoonright \beta ) \frown (q \upharpoonright [\beta,\lambda))$ decides $\sigma$ the same way.
\end{lemma}

Likewise, this proof is similar to \thref{magidor_prikry_type_sentential_better}, except with more care needed during the diagonalization step.
...
% TODO: Char'n of genericity. Basically refer up to vanilla Magidor. Here, it's like Prop 2.3 of Combos at alephomega, plus with reflection.
% c.f. meeting chat log:
\begin{comment}
11:39:11 From Dima Sinapova To Everyone : for each [b_\eta]\middle| \eta<\lambda, each [b_\eta]\in K_\eta
11:39:41 From Dima Sinapova To Everyone : for all large $\eta$, $b_\eta(\beta_\eta)\in c^*_\eta$
11:40:16 From Dima Sinapova To Everyone : rewrite the above with \xi replacing \lambda, limit \xi
11:40:29 From Dima Sinapova To Everyone : and K^\eta_\xi replacing K_\eta
11:41:16 From Dima Sinapova To Everyone : use $K_\eta<K_\xi$
11:41:41 From Dima Sinapova To Everyone : for \eta<\xi, \xi limit fixed
11:42:18 From Dima Sinapova To Everyone : K_\eta =  [f]_{U_\xi}
11:42:53 From Dima Sinapova To Everyone : look at f(\beta_\eta)
11:43:23 From Dima Sinapova To Everyone : K^\eta_\xi = f(\beta_\eta)
c^*_\eta is generic for Col(\beta_\eta^++, <\beta_{\eta+1}) 
\end{comment}
...
The following converse seems plausible:
(TODO: upgrade from conjecture to lemma, and write up a 1-ish page proof. Can even just appeal to ``like the Magidor Prikry-prop / diag'n" and run the induction argument.)
  ...
  \[
    H=\left\{(f,c,A,C) \middle|
      \begin{array}{l}
        ...
        \forall \eta \in dom(A) \ A(\eta)\in U_\eta \text{ or the relevant }r^\gamma_\eta(\beta_\eta), \\
        \forall \eta\in dom(A) \ \beta_\eta\in A(\eta), \\
        C\in V \text{ and }\forall \eta \in dom(A) \ [C(\eta)]_{U_\eta}\in K_\eta (\text{ or }r^\gamma_\eta(\beta_\eta),k^\gamma_\eta(\beta_\eta)
      \end{array}
    \right\}
  \]
...
The real work in proving this conjecture is emulating the argument of \cite{fuchs_magidor}, but with the interleaved collapses.
