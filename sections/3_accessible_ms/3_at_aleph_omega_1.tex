...
As an additional result, we state and partially prove a characterization of genericity for such forcing, along the lines of a diagonal Prikry forcing appearing in \cite{sinapova_unger_combos} and \cite{fuchs_magidor}.

Suppose $\mathrm{GCH}$; we fix $\vec{U}=\seq{U_\xi \middle| \xi\leq\lambda}$ a Mitchell order-increasing system of measure, $\lambda<\kappa$, $Y_\xi$, and $\seq{r_\beta^\alpha \middle| \beta<\alpha<\lambda}$ as in Section 1, with $\lambda\leq \omega_1$ a limit ordinal.
...
\begin{definition}\thlabel{magidor_collapse_condition_projection}
  Let $p=(f,c,A,C)\in \mathbb{M}\left(\vec{U}\right)$ and let $\xi<\lambda$.
  Then we may define the following:
  \begin{itemize}
    \item $(p)_\xi=(f,c,A,C)_\xi=(f\upharpoonright (\xi+1), c \upharpoonright (\xi+1),A\upharpoonright (\xi+1), C\upharpoonright (\xi+1))$
    \item $(p)^\xi=(f,c,A,C)^\xi=(f\upharpoonright (\lambda \setminus (\xi+1)), c \upharpoonright (\lambda \setminus (\xi+1)), A\upharpoonright (\lambda\setminus (\xi+1)), C\upharpoonright (\lambda \setminus (\xi+1)))$
    \item $\mathbb{MC}\left(\vec{U}\right)_{(\xi,\beta)}=\{(f,c,A,C)_\xi\mid (f,A)\in \mathbb{M}\left(\vec{U}\right), \xi\in dom(f), \text{ and }f(\xi)=\beta\}$
    \item $\mathbb{MC}\left(\vec{U}\right)^{(\xi,\beta)}=\{(f,c,A,C)^\xi\mid (f,A)\in \mathbb{M}\left(\vec{U}\right), \xi\in dom(f), \text{ and }f(\xi)=\beta\}$
  \end{itemize}
\end{definition}

\begin{fact}\thlabel{magidor_collapse_factoring}
  As with vanilla Magidor forcing (c.f. \thref{magidor_factoring}),
  due to our use of the reflected guiding generics $\seq{k_\alpha^\beta\middle| \alpha<\beta, \beta<\lambda}$, for each $\beta$ and each $\rho\in Y_\beta$, relative to the weakest $p$ for which $f^p(\beta)=\rho$, 
  \[
    \mathbb{MC}\left(\vec{U}\right) / p=\mathbb{MC}\left(\vec{U}\right)_{(\beta,\rho)}\times\mathbb{MC}\left(\vec{U}\right)^{(\beta,\rho)}
  \]
  so $\mathbb{MC}\left(\vec{U}\right)$ factors.
  
  In particular, 
  \[
    \mathbb{MC}\left(\vec{U}\right)_{(\beta,\rho)}=\mathbb{MC}\left(\seq{r_\alpha^\beta(\rho) \mid \alpha<\beta}\right)
  \]
  with guiding generics $\seq{k_\alpha^\beta(\rho) \mid \alpha<\beta}$,
  and
  \[
    \mathbb{MC}\left(\vec{U}\right)^{(\beta,\rho)}=\mathbb{MC}\left(\seq{U_\gamma \mid \beta<\gamma<\lambda}\right)
  \]
  (restricted to stems above $\rho$) with guiding generics $\seq{K_\gamma \mid \gamma<\lambda}$\footnote{
    We make the caveat that for $\mathbb{MC}\left(\vec{U}\right)^{(\beta,\rho)}$, we take for all conditions $p$, $f^p(-1)=\rho$, and $c^p(-1)\in Col(\rho^+,<\kappa)$.
    Otherwise the analysis for $\mathbb{MC}\left(\vec{U}\right)$ and $\mathbb{MC}\left(\vec{U}\right)^{(\beta,\rho)}$ is identical.
  }.
\end{fact}
...
\begin{definition}
  ...
  If we wish to extend by some $(\vec{\nu},c')$ such that $dom(c)=dom(\vec{\nu})$ and each $c'(\alpha)\leq C^p(\alpha)(\vec{\nu}(\alpha))$, we define $p \frown (\vec{\nu},c')$ similarly.
\end{definition}
...
\begin{proof}
  For ease of notation, let $f^p(\beta)=\rho$.
  For each $r\leq (p)_{(\beta,\rho)}$ in $\mathbb{MC}\left(\vec{U}\right)_{(\beta,\rho)}$, by \thref{magidor_collapse_prikry_open_dense_addendum}, let $p_r\leq^* r \frown (p)^\beta$ (in the full $\mathbb{MC}\left(\vec{U}\right)$) be such that for every $b$, either every $b$-step extension of $p_r$ is in $\mathcal{D}$, or none are.
  By construction, for each $r$, $(p_r)_\beta \leq^* r$ and $(p_r)^\beta = (f^p,c^{p_r},A^{p_r},C^{p_r})^\beta$.
  
  We will additionally need that the $c^{p_r}$'s are all compatible on $[\beta,\lambda)$.
  By constructing the $p_r$'s along a well-ordering of all such $r$ (of which there are at most $\rho$-many by size of  $\mathbb{MC}\left(\vec{U}\right)_{(\beta,\rho)}$), we may additionally arrange that the $c^{p_r}\upharpoonright [\beta,\lambda)$'s form a $\rho$-length descending chain in the relevant collapse forcings.
  
  For each $r$, let $q_r=(p)_\beta \frown (p_r)^\beta$; note that $q_r\leq^* p$ by construction. 
  But there are only $\rho$-many such $r$, and the $c^{p_r}$'s form a $\rho$-length descending chain in $(\rho+1)$-closed collapse forcings.
  Thus the family of $(p_r)^\beta$'s forms a $\rho$-length descending chain the $(\rho+1)$-closed forcing $\mathbb{MC}\left(\vec{U}\right)^{(\beta,\rho)}$, and thus there is a $\leq^*$-lower bound $q$ of the $q_r$'s.
  
  But then $q$ is as desired. To see this, suppose $q'\leq q$ and $q'\in\mathcal{D}$.
  Let $r=(q')_\beta$, and let $b$ be such that $q'$ is a $b$-step extension of $q$.
  Then by construction, $q'\leq r \frown (q)^\beta\leq p_r$, and by definition of $p_r$, every $b$-step extension of $p_r$, and hence any $b\setminus (dom(r^p) \cap\beta)$-step extension of $r \frown (q)^\beta$, is also in $\mathcal{D}$.
\end{proof}
...
\begin{lemma}[Prikry-type lemma, tail-change sentential version]\thlabel{magidor_collapse_prikry_sentential_better}
  Let $\sigma$ be a sentence of the forcing language, let $p\in \mathbb{MC}\left(\vec{U}\right)$, and let $\beta\in dom(f^p)$.
...
\begin{proof}
  Since the collection of conditions deciding $\sigma$ is open dense, by \thref{magidor_collapse_prikry_open_dense_better}, let $q\leq^* p$ be such that $(q)_\beta=(p)_\beta$ and whenever $r\leq^* q$ and $r\compat \sigma$, $(r)_\beta \frown(q)^\beta \compat \sigma$.
  But then $r\leq (r)_\beta \frown(q)^\beta$, so however $r$ decides $\sigma$ must also be how $(r)_\beta \frown(q)^\beta$ decides $\sigma$.
\end{proof}
...
\begin{proof}
  ...
  \[
    \mathcal{D}_4=\left\{p\in\mathbb{MC}\left(\vec{U}\right) \middle| 
      \begin{array}{l}
        \forall \eta \in dom(A^p) \setminus (\max dom(f^p)+1) \ [C^p(\eta)]_{U_\eta}<[C_\eta]_{U_\eta} \\
  ...
  \[
    \mathcal{D}_{5c}=\left\{p\in \mathbb{MC}\left(\vec{U}\right)_{(\eta,\beta_\eta)} \middle| 
      \begin{array}{l}
        \forall \zeta \in dom(A^p) \setminus (\max dom(f^p)+1) \ [C^p(\zeta)]_{r_\zeta^\eta(\beta_\eta)}<[C_\eta]_{r_\zeta^\eta(\beta_\eta)} \\
        \text{ as witnessed by }A^p(\eta)
      \end{array}
    \right\}
  \]
...
\begin{definition}[Geometricity]
  In $V^{\mathbb{MC}\left(\vec{U}\right)}$, a sequence $\vec{\beta}=\seq{\beta_\eta \mid \eta<\lambda}$ of ordinals and family of $Col(\beta_\eta^+,<\beta_{\eta+1})$-filters $\seq{F_\eta\mid \eta<\lambda}$ is \emph{geometric} if the conclusions of \thref{magidor_collapse_generic_implies_geometric} hold for $\vec{\beta}$ and $\seq{F_\eta \mid \eta<\lambda}$.
\end{definition}
  
\begin{theorem}\thlabel{magidor_collapse_generic_iff_geometric}
  In $V^{\mathbb{MC}\left(\vec{U}\right)}$, suppose $\vec{\beta}=\seq{\beta_\eta \mid \eta<\lambda}$ is increasing, normal, and has limit $\kappa$, and for each $\eta$, suppose $F_\eta$ be $Col(\beta_\eta^+,<\beta_{\eta+1})$-filters. 
...
\begin{proof}
  ...
  Our base case is when $\lambda=\omega$, and our forcing is $\mathbb{MC}\left(\seq{U_n\mid n<\omega}\right)$ with $\seq{\beta_n\mid n<\omega}$, $F_n$ for each $n<\omega$, and to which the conclusions of \thref{magidor_collapse_generic_implies_geometric} apply.
  Let $\mathcal{D}\subseteq \mathbb{MC}\left(\seq{U_n\mid n<\omega}\right)$ be open dense.
  For each $g$ a valid stem, by \thref{magidor_collapse_prikry_open_dense}, fix some $c_g$, some condition $p_{g,c_g}=(g,c_g,A^{g,c_g},C^{g,c_g})$ and an $a_g\in [\omega]^{<\omega}$ such that every $a_g$-step extension of $p_{g,c_g}$ is in $\mathcal{D}$.
  Without loss of generality, we suppose that $a_g=[lh(g),lh(g)+m_g)$ for some $m_g\in \omega$.
  
  By arguing as in \thref{magidor_collapse_meets_dense_generic_one_step}, we may choose $c_g$'s such that for each $a\in[\omega]^{<\omega}$ and each $n\in a$,
  $[c_g(\alpha)\mid dom(g)=a]_{U_n}\in K_n$.
  Thus by \thref{magidor_collapse_modified_diagonalization}, there is a condition $p$ with $f^p=((-1,\omega_1))$ such that for every $g$ for which $p\frown g\leq p$, $p\frown g \leq^* p_{g,c_g}$.
  
  By geometricity for $\vec{\beta}$ (namely Item \ref{magidor_collapse_generic_implies_geometric:measures}) and for the collapses (Item \ref{magidor_collapse_generic_implies_geometric:collapse_futures}, there is an $m\in \omega$ such that for all $n\geq m$, $\beta_n\in A^p(n)$ and $C^p(n)(\beta_n)\in F_n$. 
  
  Let $g=\vec{\beta}\upharpoonright [-1,m-1]$.
  For each $l\leq m$, $c_g(l)$ and $C^p(l)(g(l))$, by adjusting measure one sets as needed above, are compatible, so let $c'_g(l)= c_g(l)\land C^p(l)(g(l))$.
  Let $q=(g,c'_g,A^p \setminus m, C^p \setminus m)$.
  By construction, $q\leq p_{g,c_g}$.
  Let $r=q\frown (\vec{\beta}\upharpoonright m_g)$; by construction of $p_{g,c_g}$, $r\in\mathcal{D}$.
  
  To argue that $r\in H$, note that by construction, for all $n\geq lh(r)$, $\beta_n\in A^r(n)$ and $C^r(n)(\beta_n)\in F_n$.
  So all we need to do is argue that for each $l<lh(c^r)$, $c^r(l)\in F_l$.
  For $l>lh(g)$, $c^r(l)=C^q(l)\in F_l$ by geometricity.
  For $l\leq lh(g)$, by the genericity of the $F_n$'s, and since our choices for $c_g$ ranged over a dense subset of $\prod_{l\leq lh(g)} Col(\beta_l^+,\beta_{l+1})$, we can arrange to have $c^r(l)\in F_l$.
  This concludes the base case.
  
  For the inductive steps, the argument proceeds almost exactly as in the Magidor forcing (c.f. \thref{magidor_geometric_iff_generic}).
  
  For the successor step, suppose that $\lambda=\bar{\lambda}+\omega$ and suppose the result is true for any forcing of the form $\mathbb{MC}\left(\seq{U'_\eta \mid \eta\leq \bar{\lambda}}\right)$.
  Let $\vec{U}=\seq{U_\eta \mid \eta\leq \lambda}$, and let $\vec{\beta}=\seq{\beta_\eta \mid \eta<\lambda}$ and $\seq{F_\eta \mid \eta<\lambda}$ be geometric for $\mathbb{MC}\left(\vec{U}\right)$, with induced filter $H$.
  ...
  Then the induction hypothesis applies to $\mathbb{MC}\left(\vec{U}\right)_g=\mathbb{MC}\left(\seq{r_\eta^{\bar{\lambda}}(\beta_{\bar{\lambda}}) \mid \eta\leq \lambda}\right)$, 
  ...
  Our induction hypothesis is now that for each $\rho<\tau$ and for each $\seq{\beta_\eta \mid \eta<\lambda_\rho}$ and $\seq{F_\eta \mid \eta<\lambda_\rho}$ geometric for some Magidor collapse forcing $\mathbb{MC}\left(\seq{U_\eta \mid \eta\leq \lambda_\rho}\right)$, the resulting filter $H_\rho$ is $\mathbb{MC}\left(\seq{U_\eta \mid \eta\leq \lambda_\rho}\right)$-generic over $V$;
  ...
  By \thref{magidor_collapse_prikry_open_dense_better}, for each $g=(f,c)$ such that $f$ is a stem and $c$ is a compatible collapsing sequence\footnote{
    that is, there is some condition $p$ such that $f^p=f$ and $c^p=c$
  },
  take $p_g=(f,c,A_g,C_g)$ such that
  \begin{itemize}
    \item $p_g\in \mathbb{MC}\left(\seq{U_\eta \mid \eta\leq\lambda}\right)$
    \item letting $\delta=\max dom(f)$, $(p_g)_\delta = \left(1_{\mathbb{MC}\left(\seq{U_\eta \mid \eta\leq\lambda}\right)}\right)_\delta$
    \item (tail-change property) for every $b\subseteq [dom(A_g)]^{<\omega}$, if $q$ is a $b$-step extension of $p_g$ and $q\in\mathcal{D}$ then every $b\setminus (dom(f^q)\cap \delta)$-step extension of $(q)_\delta \frown (p_g)^\delta$ is also in $\mathcal{D}$
  \end{itemize}
  By arguing as in \thref{magidor_collapse_meets_dense_generic_one_step}, we may restrict our attention to a subfamily of the $p_{(f,c)}$'s (including every possible $f$) which satisfies the hypotheses of \thref{magidor_collapse_modified_diagonalization}.
  So by \thref{magidor_collapse_modified_diagonalization}, let $q$ be such that $f^q=\seq{(-1,\omega_1)}$ and whenever $g$ is a valid stem and collapse term by which to extend $q$, $q \frown g\leq^* p_g$.
  ...
  By the induction hypothesis appled to $\seq{\beta_\eta \mid \eta<\lambda_\rho}$ and $\seq{F_\eta \mid \eta<\lambda_\rho}$, we induce $H_\rho$ a $\mathbb{MC}\left(\seq{r_\eta^{\lambda_\rho}(\beta_{\lambda_\rho}) \mid \eta\leq \lambda_\rho}\right)$-generic filter over $V$.
  
  Let
  \[
    \mathcal{D}'=\left\{ (p)_{\lambda_\rho} \mid p\in \mathcal{D}, (p)^{\lambda_\rho}\leq (q)^{\lambda_\rho}, \text{ and }f^p(\lambda_\rho)=\beta_{\lambda_\rho}\right\}
  \]
  (Recall that $(p)_{\lambda_\rho}$ is essentially $p\upharpoonright (\lambda_\rho + 1)$; see \thref{magidor_collapse_condition_projection} for a complete description.)
  
  Since $c^q(-1)=1_{Col(\omega_2,<\kappa)}$, $\mathcal{D}'$ is open dense in $\mathbb{MC}\left(\seq{r_\eta^{\lambda_\rho}(\beta_{\lambda_\rho}) \mid \eta\leq \lambda_\rho}\right)$, let $r\in\mathcal{D}$ be such that $r\leq q$ and $(r)_{\lambda_\rho}\in H_\rho\cap \mathcal{D}'$ and for all $\xi\geq \lambda_\rho$, $\beta_\xi\in A^r(\xi)$ and $C^r(\xi)(\beta_\xi)\in F_\xi$.
  
  Let $g=(f^r \upharpoonright (\lambda_\rho+1),c\upharpoonright (\lambda_\rho+1))$.
  Then by construction of $r$, $r\leq p_g$; however, $r\in \mathcal{D}$, so letting $b=dom(r)\setminus dom(f^{p_g})$, by the tail-change property of $p_g$, every $b$-step extension of $(r)_{\lambda_\rho} \frown (p_g)^{\lambda_\rho}$ is also in $\mathcal{D}$.
  Let $r'=((r)_{\lambda_\rho} \frown (p_g)^{\lambda_\rho}) \frown (\vec{\beta}\upharpoonright b)$.
  Then $r'$ is a $b$-step extension of $(r)_{\lambda_\rho} \frown (p_g)^{\lambda_\rho}$, so lies in $\mathcal{D}$.
  By construction, $f^{r'}$ is a finite subsegment of $\vec{\beta}$. 
  Also by construction, for each relevant $\xi$, $\beta_\xi\in A^{r'}(\xi)$, and $C^{r'}(\xi)(\beta_\xi)\in F_\xi$.
  By induction and construction of $r$ and $q$, for each relevant $\xi$, $c^{r'}(\xi)\in F_\xi$.
  Thus $r'\in H$, and so $r'\in H \cap\mathcal{D}$.
  This completes the limit step.
\end{proof}

\begin{conjecture}
  In $V^{\mathbb{MC}\left(\vec{U}\right)}$ with $\lambda=\omega_1$, 
  \[
