...
If $c$ is a family of collapse terms, we will additionally need a notion of $p\frown (\vec{\nu},c)$:
...
\begin{definition}
  ...
  \end{itemize}
  If in addition $c=(\alpha,r)$ for some $r\leq C^p(\alpha)(\nu)$, then we define $p\frown ((\alpha,\nu),c)$ as the weakest direct extension of $p\frown (\alpha,\nu)$ such that $c^{p\frown ((\alpha,\nu),c)}(\alpha)=r$.
  
  For longer $\nu$, we define, for each $a\in [dom(A^p)]^{<\omega}$ and each strictly increasing $\vec{\nu}\in \prod_{\alpha\in a} A^p(\alpha)$, $p\frown \vec{\nu}$ similarly.
  
  For larger $c$, if $dom(c)\subseteq dom(\vec{\nu})\cup dom(f^p)$ such that for each $\alpha\in dom(\vec{\nu})\cap dom(c)$, $c(\alpha)\leq C^p(\alpha)(\nu)$, and for each $\alpha\in dom(f^p)\cup dom(c)$, $c(\alpha)\leq c^p(\alpha)$,
  then we define $p\frown (\vec{\nu},c)$ as the weakest direct extension of $p\frown \vec{\nu}$ such that for all $\alpha\in dom(c)$, $c^{p\frown (\vec{\nu},c)}(\alpha)=c(\alpha)$.
\end{definition}
...
\begin{proof}
  ...
  Without loss of generality, we suppose that $a_g=[lh(g)-1,lh(g)+m_g-1)$ for some $m_g\in \omega$.
  ...
  What we actually need is $q$ a direct extension of $p$ with the following property: for any $g$, $(g,c_g) \frown (q)^{\max dom(g)}\leq^* p_{g,c_g}$.
  To attain this, we define $f^q=f^{p}$ and $c^q=c^{p}$.
  As for $A^q$ and $C^q$, let
  \[
    B(n)=A^{p}(n) \cap \diagcap_{g}A^{p_{g,c_g}}(n)=A^{p}(n) \cap \left\{\alpha<\kappa \mid \forall g \ |g|<n, \max ran(g)<\alpha\implies \alpha\in A^{p_{g,c_g}}(n)\right\}
  \]
  For all $n$ observe that $\{[C^{p_{g,c_g}}(n)] \mid |g|<n\}$ forms a $\kappa$-sized directed set in a $\kappa^+$-distributive collapse forcing.
  So let $C'(n)$ be such that $[C'(n)]$ is a lower bound of the $[C^{p_{g,c_g}}(n)]$'s as witnessed by $W_n$.
  Then let $A^q(n)=B(n)\cap W_n$, and let $C^q(n)=C' \upharpoonright A^q(n)$.
  Then by construction, for any $g$, $(g,c_g) \frown (q)^{\max dom(g)}\leq^* p_{g,c_g}$.
  ...
  By construction, $(g,c_g) \frown (p)^{m-1} \leq p_{g,c_g}$.
  Let $r=q\frown (\vec{\beta}\upharpoonright [m,m_g])$; by construction of $p_{g,c_g}$, $r\in\mathcal{D}$.
  
  Next we argue that we can arrange our argument so that $r\in H$. 
  Note that by construction, for all $n\geq lh(r)$, $\beta_n\in A^r(n)$ and $C^r(n)(\beta_n)\in F_n$.
  So all we need to do is arrange that for each $l<lh(c^r)$, $c^r(l)\in F_l$.
  For $l>lh(g)$, this is already guaranteed: $c^r(l)=C^q(l)$ by definition of $r$, and $C^q(l)\in F_l$ by geometricity.
  ...
  By applying \thref{magidor_collapse_prikry_open_dense_better} to the trivial condition, for each $g=(f,c)$ such that $f$ is a stem for which $\max dom(f)=\delta+1$ for some $\delta$ such that $\delta\in dom(f)$,
  and $c$ is a compatible collapsing sequence\footnote{
  ...
  \begin{itemize}
    ...
    \item letting $\delta+1=\max dom(f)$, $(p_g)_{\delta+1} = \left(1_{\mathbb{MC}\left(\seq{U_\eta \mid \eta<\lambda}\right)}\right)_{\delta+1} \frown g$
    \item (tail-change property) for every $b\subseteq [dom(A_g)]^{<\omega}$, if $q$ is a $b$-step extension of $p_g$ and $q\in\mathcal{D}$ then every $b\setminus (dom(f^q)\cap (\delta+1)$-step extension of $(q)_{\delta+1} \frown (p_g)^{\delta+1}$ is also in $\mathcal{D}$
  \end{itemize}
