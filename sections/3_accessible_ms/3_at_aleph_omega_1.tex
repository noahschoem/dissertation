...
As an additional result, we state and partially prove a characterization of genericity for such forcing, along the lines of (todo: cite Sinapova-Unger, Combinatorics at $\aleph_\omega$) and (todo: cite Fuchs).
...
\begin{proof}
  ...
  As for obtaining that $K_\alpha\in Ult(V,U_\beta)$, observe that since $U_\alpha\in Ult(V,U_\beta)$ (and by $GCH$), we simply have that $\left| j_{U_\alpha}(\kappa) \right|^{Ult(V,U_\beta)}=2^\kappa=\kappa^+$ (by $\kappa^\kappa$ onto $j_{U_\alpha}(\kappa)$ by $f \mapsto [f]_{U_\alpha}$, see for instance (Jech, arg that no n.m. is in its own ultrapower) for details).
...
\begin{remark}
  For ease of notation, and in accordance with conventions found in (handbook chapter), we may write a condition $p\in \mathbb{MC}(\vec{U})$ as
  \[
    p=(f^p,c^p,A^p,C^p)
  \]
