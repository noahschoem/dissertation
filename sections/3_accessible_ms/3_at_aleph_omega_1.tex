...
\begin{fact}\thlabel{magidor_collapse_factoring}
  ...
  Thus, as with $\mathbb{M}(\vec{U})$, many preservation arguments that work for $\mathbb{MC}\left(\vec{U}\right)$ still reflect downwards.
\end{fact}
...
As for cardinal arithmetic in $V^{\mathbb{MC}\left(\vec{U}\right)}$, we have a Magidor sequence $\seq{\beta_\eta \middle| \eta<\lambda}$, collapses of cardinals in $(\beta_\eta^+,\beta_{\eta+1})$ for every $\eta \in \lambda \cup\{-1\}$, and preservation of all other cardinals. 
Explicitly:
...
\begin{proof}
  ...
  In the casee of \ref{magidor_collapse_generic_implies_geometric:refl_measures}, we factor to $\mathbb{MC}\left(\vec{U}\right)_{(\eta,\beta_\eta)}$, which is its own Magidor forcing with guided interleaved collapses.
  ...
  \[
    \left\{p\in\mathbb{MC}\left(\vec{U}\right) \middle| \forall \eta \in dom(A^p) \setminus (\max dom(f^p)+1) \ [C^p(\eta)]_{U_\eta}<[C_\eta]_{U_\eta} \right\}
  \]
  is dense.
  ...
  Then 
  \[
    \left\{p\in \mathbb{MC}\left(\vec{U}\right)_{(\eta,\beta_\eta)} \middle|  \forall \zeta \in dom(A^p) \setminus (\max dom(f^p)+1) \ [C^p(\zeta)]_{r_\zeta^\eta(\beta_\eta)}<[C_\eta]_{r_\zeta^\eta(\beta_\eta)}\right\}
  \]
  is dense in $\mathbb{MC}\left(\vec{U}\right)_{(\eta,\beta_\eta)}$.
\end{proof}
  
As with Magidor forcing, the above is a precise characterization of genericity for $\mathbb{MC}\left(\vec{U}\right)$:
  
\begin{theorem}
  In $V^{\mathbb{MC}\left(\vec{U}\right)}$, suppose $\seq{\beta_\eta \mid \eta<\lambda}$ is increasing, normal, and has limit $\kappa$, and for each $\eta$, suppose $F_\eta$ be $Col(\beta_\eta^+,<\beta_{\eta+1})$-generic over $V$.
