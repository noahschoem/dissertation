...
\begin{lemma}[Prikry-type lemma, tail-change version]\thlabel{magidor_collapse_prikry_open_dense_better}
  ...
  then there is a direct extension $q\leq^* p$ such that $(q)_{\beta+1} = (p)_{\beta+1}$ and whenever $a\in [dom(A^p)]^{<\omega}$, if $r$ is an $a$-step extension of $q$ and $r\in \mathcal{D}$, then every $a\setminus (dom(r^p)\cap (\beta+1))$-step extension of $(r)_{\beta+1} \frown (q)^{\beta+1}$ is also in $\mathcal{D}$.
\end{lemma}
...
\begin{proof}
  ...
  Then by construction, $q'\leq r \frown (q)^{\beta+1}\leq p_r$, and by definition of $p_r$, every $b$-step extension of $p_r$, and hence any $b\setminus (dom(r^p) \cap(\beta+1))$-step extension of $r \frown (q)^{\beta+1}$, is also in $\mathcal{D}$.
\end{proof}
Likewise, we have a similar sentential version with tail changes:

% TODO: propagate domain change from magidor_prikry_type_open_dense_better
