...
\begin{proof}
  ...
  By Item \ref{magidor_collapse_generic_implies_geometric:collapses}, by arguing as in \thref{magidor_collapse_meets_dense_generic_one_step}, we may choose $c_g$'s such the family of $\seq{p_{g,c_g}\mid g\text{ a stem}}$ meets the necessary conditions for our diagonalization lemma \thref{magidor_collapse_modified_diagonalization}.
  Thus by \thref{magidor_collapse_modified_diagonalization}, there is a condition $p$ with $f^p=((-1,\lambda))$ such that for every $g$ for which $p\frown g\leq p$, $p\frown g \leq^* p_{g,c_g}$.
  ...
  Our induction hypothesis is now that for each $\rho<\tau$ and for each $\seq{\beta_\eta \mid \eta<\lambda_\rho}$ and $\seq{F_\eta \mid \eta<\lambda_\rho}$ geometric for some Magidor collapse forcing $\mathbb{MC}\left(\seq{U_\eta \mid \eta< \lambda_\rho}\right)$, the resulting filter $H_\rho$ is $\mathbb{MC}\left(\seq{U_\eta \mid \eta\leq \lambda_\rho}\right)$-generic over $V$;
  ...
  \begin{itemize}
    ...
    \item letting $\delta=\max dom(f)$, $(p_g)_\delta = \left(1_{\mathbb{MC}\left(\seq{U_\eta \mid \eta<\lambda}\right)}\right)_\delta$
  ...
  \end{itemize}
  By arguing as in \thref{magidor_collapse_meets_dense_generic_one_step}, we may choose for every $f$ a $c_f$ such that $\seq{p_{(f,c_f)} \mid f\text{ a stem}}$ satisfies the hypotheses of \thref{magidor_collapse_modified_diagonalization}.
  (Note that before we used the notation $\vec{\nu}$ for $f$.)
  So by \thref{magidor_collapse_modified_diagonalization}, let $q$ be such that $f^q=\seq{(-1,\lambda)}$ and whenever $f$ is a valid stem by which to extend $q$, $q \frown (f,c_f)\leq^* p_g$.
  ...
  By the induction hypothesis appled to $\seq{\beta_\eta \mid \eta<\lambda_\rho}$ and $\seq{F_\eta \mid \eta<\lambda_\rho}$, we induce $H_\rho$ a $\mathbb{MC}\left(\seq{r_\eta^{\lambda_\rho}(\beta_{\lambda_\rho}) \mid \eta<\lambda_\rho}\right)$-generic filter over $V$.
  ...
  Since $c^q(-1)=1_{Col(\lambda^+,<\kappa)}$, $\mathcal{D}'$ is open dense in $\mathbb{MC}\left(\seq{r_\eta^{\lambda_\rho}(\beta_{\lambda_\rho}) \mid \eta<\lambda_\rho}\right)$, let $r\in\mathcal{D}$ be such that $r\leq q$ and $(r)_{\lambda_\rho}\in H_\rho\cap \mathcal{D}'$ and for all $\xi\geq \lambda_\rho$, $\beta_\xi\in A^r(\xi)$ and $C^r(\xi)(\beta_\xi)\in F_\xi$.
