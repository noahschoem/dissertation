...
\begin{proposition} \thlabel{magidor_collapse_generic_implies_geometric}
  ...
  Then
  \begin{enumerate}
    \item $\vec{\beta}$ is increasing, normal, and has supremum $\kappa$
  
    \item for all $\seq{A_\eta \middle| \eta<\lambda}\in V$ with each $A_\eta\in U_\eta$, for coboundedly many $\zeta<\lambda$, $\beta_\zeta\in A_\zeta$
    
    \item for all $\eta$, $F_\eta$ is $Col(\beta_\eta^+,<\beta_{\eta+1})$-generic
...
\begin{proof}
  These all follow from standard density and factoring arguments (c.f. \thref{magidor_generic_implies_geometric}), but with collapses included.
